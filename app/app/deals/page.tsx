...
export const metadata = { title: 'Ponude — balkan.works' }
  ...
  return (
    <div className="space-y-6 px-4 py-6 pb-24">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1" aria-label="Kategorije ponuda">
        {['Sve', 'Hrana', 'Dom', 'Auto', 'Lepota'].map((filter, index) => (
          <button key={filter} type="button" className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-medium ${index === 0 ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-muted-foreground'}`}>{filter}</button>
        ))}
      </div>
        ...
        {deals.map((deal) => (
          <article key={deal.id} className="rounded-2xl border border-border bg-card p-4 shadow-soft">
            ...
            <div className="mt-4 flex gap-2 border-t border-border pt-3">
              ...
              <button type="button" aria-label={`Sačuvaj ponudu ${deal.title}`} className="grid size-10 place-items-center rounded-xl border border-border text-muted-foreground transition hover:border-primary hover:text-primary"><Bookmark className="size-4" /></button>
            </div>
          </article>
        ))}
      </section>
