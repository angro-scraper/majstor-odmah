...
This is the active Balkan.works TypeScript monorepo. The previous Majstor Odmah application and its prototypes are preserved in `archive/majstor-odmah-legacy/` and are not part of the active product or deployment.
...
`render.yaml` is the production blueprint at the repository root. It creates a public Next.js web service, a NestJS API service and a managed PostgreSQL database. The API verifies availability at `/api/v1/health`.
